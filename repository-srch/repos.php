...
function denyBots() {
  if(!empty($_GET['comment'])) die();
}

function getGetVars() {
  $parameters = array(
    0 => !empty($_GET['term']) ? $_GET['term'] : 'wordpress',
    array(
      'language' => !empty($_GET['language']) ? $_GET['language'] : 'php',
      'start_page' => REPO_START_PAGE
    )
  );
  return $parameters;
}
...
function redirectOnFinish() {
  if(isset($_GET['redirect'])) {
    ...
    echo 'All done. Go check the database.';
  }
}
